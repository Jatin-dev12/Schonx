...
function ScrollTab() {
  ...
  return (
    <section className='what-we-do' style={{ backgroundImage: `url('/images/white_bg.jpg')` }}>
      <div className='container-fluid'>
        <div className='row row_justify'>
          <div className='col-lg-12 col-md-12'>
            <div className='main-heading'>
              ...
              <p className='para'>
                Crafted for Performance. Designed to Inspire Our Range of Precision-Engineered Solutions
              </p>
            </div>
          </div>
        ...
        <div className='row-what-we-do'>
          ...
          <div className='row-what-we-do-inner-2' ref={containerRef}>
            {/* Repeatable Sections */}
            {[{ name: 'All Projects' }, ...categories].map((cat, i) => {
              const currentProjects =
                cat.name === 'All Projects'
                  ? projects
                  : projects.filter((project) => project.category.name === cat.name);

              return (
                <div
                  key={cat.name}
                  id={cat.name}
                  className={`row-what-we-do-inner-3 ${activeId1 === cat.name ? 'active' : ''}`}
                >
                  <div className='small-para'>
                    <p className='small-inpara'>{getCategoryDescription(cat.name)}</p>
                  </div>
                  ...
                  <div className='row row-inn-img'>
                    {currentProjects.slice(0, 4).map((project, index) => (
                      <div key={project.id} className='col-lg-3 col-md-6 col-sm-12'>
                        <Link href={`/products-detail?slug=${project.slug}`} style={{ textDecoration: 'none' }}>
                          <div className='inner-wedo'>
                            <div className='inner-wedo-img'>
                              <img src={project.hero_img || placeholderImages[index % 4]} alt={project.name} />
                            </div>
                  ...
                  <div className='row'>
                    <div className='header-inner-3'>
                      <Link className='header-btn' href={cat.slug ? `/products?slug=${cat.slug}` : '/products'}>
